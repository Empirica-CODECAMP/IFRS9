...
list.of.packages <- c("dplyr", "here", "data.table", "readr", "lubridate", "RODBC", "reshape", "stringr", "openxlsx", "quantmod", "ggplot2", "tools", "readxl", "bit64")
...
library("bit64")
library(here)
here::here("calculations/Rscript")
...
NBRun_Nr <- as.integer(args[2])
PrevRun_Nr <- as.integer(args[3])

# Run_Nr <- 2024
# PrevRun_Nr <- 2024
# NBRun_Nr <- 2024
#
Portf <- "BLL" #readxl::read_excel(excel_file, sheet = "Setup", range = "B27", col_names = FALSE)

Stress <- tryCatch(
  {
    # Attempt to read the value from args[4]
    as.character(args[4])
  },
  error = function(e) {
    # Return "Base" if an error occurs
    "Base"
  }
)

# Validate the Stress value
if (is.null(Stress) || Stress == "" || all(is.na(Stress))) {
  Stress <- "Base"
}
# Log a message if Stress is set to "Base"
if (Stress == "Base") {
  message("Failed to read a valid value, setting Stress to 'Base'.")
}
...
base_dir <- here("calculations/Rscript/Inputs", Stress)
...
main_folder <- here("calculations/Rscript/Output")
...
# Directory paths
excel_directory <- here("calculations/Rscript/Assumptions/TABLES/Curves", Stress, "Excel/")
Csv_directory <- here("calculations/Rscript/Assumptions/TABLES/Curves", Stress, "Csv")
...
for (file_path in excel_files) {
  ...
  csv_path <- here("calculations/Rscript/Assumptions/TABLES/Curves", Stress, paste0(file_name, ".csv"))
...
root_dir <- here("calculations/Rscript/Inputs", Stress, "Excel/NB/Insurance")
...
output_dir <- file.path(here("calculations/Rscript/Inputs", Stress, "NB/Insurance"), as.character(NBRun_Nr))
...
# Specify the root directory containing subdirectories with Excel files
root_dir <- here("calculations/Rscript/Inputs", Stress, "Excel/IF/Insurance")
...
output_dir <- file.path(here("calculations/Rscript/Inputs", Stress, "IF/Insurance"), Run_Nr)
...
root_dir <- here("calculations/Rscript/Inputs", Stress, "Excel/NB/Reinsurance")
...
output_dir <- file.path(here("calculations/Rscript/Inputs", Stress, "NB/Reinsurance"), NBRun_Nr)
if (!dir.exists(output_dir)) {
  dir.create(output_dir, recursive = TRUE)
}
setwd(output_dir)
...
# Specify the root directory containing subdirectories with Excel files
root_dir <- here("calculations/Rscript/Inputs", Stress, "Excel/IF/Reinsurance")
root_dir <- file.path(root_dir, as.character(NBRun_Nr))
...
output_dir <- file.path(here("calculations/Rscript/Inputs", Stress, "IF/Reinsurance"), Run_Nr)
if (!dir.exists(output_dir)) {
  dir.create(output_dir, recursive = TRUE)
}
setwd(output_dir)
...
GrossCSM <- function(PrevRun_Nr, NBRun_Nr, Run_Nr) {
  ...
  ParameterTable <- data.frame(read_csv(
    file = paste0("calculations/Rscript/Parameters/", "ParameterTable.csv"),
    col_types = cols(
  ...
  EconomicAssumptions <- data.frame(read_csv(
    file = paste0("calculations/Rscript/Assumptions/TABLES/Economic/", EconomicAssumptionsName),
    col_types = cols(
      Value_D = col_date(format = "%Y/%m/%d"),
      Value_N = col_double(),
      Value_C = col_character()
  ...
  # Interest rates
  # ForwardInterestRates <- data.frame(read_csv(paste0("calculations/Rscript/Assumptions/TABLES/Curves/",ForwardInterestRatesName),
  #                                             col_types = cols(ProjM              = col_integer(),
  #                                                              NominalForwardRate = col_double(),
  #                                                              RealForwardRate    = col_double())))
  #
  ForwardInterestRates <- data.frame(read_csv(paste0("calculations/Rscript/Assumptions/TABLES/Curves/", Stress, "/", ForwardInterestRatesName),
    col_types = cols(
      ProjM = col_integer(),
      NominalForwardRate = col_double(),
      RealForwardRate = col_double()
  ...
  outwd <- file.path(inwd, paste0("calculations/Rscript/Output/", NBRun_Nr, "/", Portf, "/", Stress, "/NB/Insurance"))
  ...
  run_dir <- file.path(inwd, "calculations/Rscript/Inputs", Stress, "NB/Insurance", NBRun_Nr)
  setwd(run_dir)
  ...
  url <- "http://127.0.0.1:8000/configuration/insurance_variables/"
  ...
  for (i in 1:length(files)) {
    ...
    for (premium_col in FCFVars$premium) {
    ...
    # Calculate 'Adm' using columns specified in FCFVars$admin
    for (adm_col in FCFVars$admin) {
    ...
    # Calculate 'Acq' using columns specified in FCFVars$acquisition
    for (acq_col in FCFVars$acquisition) {
  ...
  output_dir <- here(paste0("calculations/Rscript/Output/", Run_Nr, "/", Portf, "/", Stress, "/Reports"))
  output_file <- here(paste0("calculations/Rscript/Output/", Run_Nr, "/", Portf, "/", Stress, "/Reports/Report 1.3 NB Recognition.xlsx"))

  # Create the directory if it does not exist
  if (!dir.exists(output_dir)) {
    dir.create(output_dir, recursive = TRUE)
  }
  wb <- loadWorkbook(here("calculations/Rscript/Reports/Templates/Report 1.3 NB Recognition.xlsx"))
  ...
  # }
  run_dir <- paste0(here("calculations/Rscript/Output"), "/", Run_Nr, "/", Portf, "/", Stress, "/CFS/NB/Insurance/NB Insurance CFs")
  ...
  outwd <- file.path(inwd, "calculations/Rscript/Output", Run_Nr, Portf, Stress, "IF")
  file_list <- list.files(outwd, full.names = TRUE)
  file.remove(file_list)

  run_dir <- file.path(inwd, "calculations/Rscript/Inputs", Stress, "IF/Insurance", Run_Nr)
  if (!dir.exists(run_dir)) {
    dir.create(outwd, recursive = TRUE)
  }
  setwd(run_dir)
  ...
  for (i in 1:length(files)) {
    ...
    filepath <- file.path(here("calculations/Rscript/Assumptions/TABLES/Curves", Stress), ForwardInterestRatesNameLockedCohort)
  ...
  outwd <- file.path(inwd, paste0("calculations/Rscript/Output/", Run_Nr, "/", Portf, "/", Stress, "/Locked/IF/Insurance"))
  if (!dir.exists(outwd)) {
    dir.create(outwd, recursive = TRUE)
  }
  setwd(outwd)
  ...
  # }
  ######################################################### CSM run-off report################################################################
  wb <- loadWorkbook(here("calculations/Rscript/Reports/Templates/Report 1.2 CSM run-off.xlsx"))
  ...
  output_dir <- here(paste0("calculations/Rscript/Output/", Run_Nr, "/", Portf, "/", Stress, "/Reports"))
  output_file <- here(paste0("calculations/Rscript/Output", Run_Nr, "/", Portf, "/", Stress, "Reports/Report 1.2 CSM run-off.xlsx"))
  if (!dir.exists(output_dir)) {
    dir.create(output_dir, recursive = TRUE)
  }

  saveWorkbook(wb, output_file, overwrite = TRUE)
  run_dir <- paste0(here("calculations/Rscript/Output"), "/", Run_Nr, "/", Portf, "/", Stress, "/CFS/Locked/IF/Insurance/IF Insurance CFs")
  ...
  inwd <- here("calculations/Rscript/")
  ...
  for (i in 1:length(files)) {
    ...
    # Calculate 'Premiums'
    # Premiums <- as.numeric(file$PREM_INC) + as.numeric(file$V_PREM_INC)
    # Calculate 'Premiums' using columns specified in FCFVars$premium
    for (premium_col in FCFVars$premium) {
      if (premium_col %in% names(file)) {
        Premiums <- Premiums + as.numeric(file[[premium_col]])
      } else {
  ...
  run_dir <- paste0(here("calculations/Rscript/Output"), "/", Run_Nr, "/", Portf, "/", Stress, "/CFS/IF/Insurance/IF Insurance CFs")
...
ReinsCSM <- function(PrevRun_Nr, NBRun_Nr, Run_Nr) {
  ...
  url <- "http://127.0.0.1:8000/configuration/reinsurance_variables/"
  ...
  run_dir <- paste0(here("calculations/Rscript/Output"), "/", Run_Nr, "/", Portf, "/", Stress, "/CFS/NB/Reinsurance/NB Reinsurance CFs")
  ...
  saveWorkbook(wb, here(paste0("calculations/Rscript/Output/", Run_Nr, "/", Portf, "/", Stress, "/Reports/Report 1.2 CSM run-off.xlsx")), overwrite = TRUE)
  ...
  run_dir <- file.path(inwd, "calculations/Rscript/Inputs", Stress, "IF/Reinsurance", Run_Nr)
  if (!dir.exists(run_dir)) {
    dir.create(outwd, recursive = TRUE)
  }
  setwd(run_dir)
  ...
  outwd <- file.path(inwd, "calculations/Rscript/Output", Run_Nr, Portf, Stress, "IF/Reinsurance")
  if (!dir.exists(outwd)) {
    dir.create(outwd, recursive = TRUE)
  }
  setwd(outwd)
  ...
  run_dir <- paste0(here("calculations/Rscript/Output"), "/", Run_Nr, "/", Portf, "/", Stress, "/CFS/IF/Reinsurance/IF Reinsurance CFs")
...
setwd(paste0(here("calculations/Rscript/Output"), "/", Run_Nr, "/", Portf, "/", Stress))
output_file <- paste0(here("calculations/Rscript/Output"), "/", Run_Nr, "/", Portf, "/", Stress, "/Master_Results.xlsx")
...
main_folder <- paste0(here("calculations/Rscript/Output"), "/", Run_Nr, "/", Portf, "/", Stress)
...
setwd(paste0(here("calculations/Rscript/Output"), "/", Run_Nr, "/", Portf, "/", Stress, "/CFS"))
output_file <- paste0(here("calculations/Rscript/Output"), "/", Run_Nr, "/", Portf, "/", Stress, "/Master_Results.xlsx")
...
combine_and_write_to_excel(paste0(here("calculations/Rscript/Output"), "/", Run_Nr, "/", Portf, "/", Stress, "/CFS"), output_file, Run_Nr, Portf)
...
wd <- paste0(here("calculations/Rscript/Output"), "/", Run_Nr, "/", Portf, "/", Stress)
# Define file paths
...
excel_path <- here("calculations/Rscript/Reports/Templates/IFRS17_Master_Results.xlsx")
ifrs_output <- file.path(here("calculations/Rscript/Output"), Run_Nr, Portf, Stress, "Reports/IFRS17_Master_Results.xlsx")
# Load the workbook
